...
- **歌词缓存**：播放过的歌词自动缓存到本地，离线也可显示已缓存歌词
...
2. core/netease_source.py 在切歌后优先读取缓存，若未命中则请求 LRC，并自动缓存结果。
...
6. core/updater.py 启动后检测新版本，支持一键下载安装。
7. core/color_analyzer.py 从封面提取颜色，实现自动主题色。
