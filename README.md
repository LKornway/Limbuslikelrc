...
- **双语字幕**：设置中可选择「原文 / 翻译 / 双语」三种显示模式；双语时原文与翻译同框分两行展示（同字号同外观），
  翻译逐字节奏与原句同步（第 i 个字符同时出现）；原文折行时翻译顺延到其下方
  - 网易云音乐：官方翻译歌词（`tlyric`）
  - 酷狗音乐：歌词文件内嵌 `[language]` 多语言歌词（按行序对应）
  - QQ 音乐：匿名接口不下发翻译，采用**跨源译文兜底**（同名歌曲译文 + 原文与时间轴双重校验，校验不通过则不显示翻译）
...
- 歌词显示：可选择「原文 + 翻译（双语）」/「仅原文」/「仅翻译」；双语时翻译跟随原文之下、同字号同外观（默认双语）。
...
| 平台 | 歌曲/切歌识别 | 播放进度 | 歌词 | 译文（双语字幕） | 封面 |
|---|---|---|---|---|---|
| 网易云音乐 | 解析本地 `cloudmusic.elog`（桌面版/Store 版路径自动探测） | 日志实时进度（支持拖动/中途启动） | 网易云 LRC 接口 + 本地缓存 | 官方 `tlyric`（时间戳配对，100% 对齐） | 网易云接口 240×240 |
| QQ 音乐 | SMTC 会话（标题/歌手/专辑，专辑用于同名版本去歧义） | SMTC 实时进度 | `lyric_new` 接口 + 本地缓存（songmid 键） | 跨源兜底：同名歌曲译文 + 原文/时间轴双重校验 | SMTC 封面流 |
| 酷狗音乐 | 主窗口标题 `歌手 - 歌名 - 酷狗音乐`（兼容大小窗口） | **本地时钟累计**（见限制） | 本地 `.krc` 解密 / 在线歌词接口（hash 键） | KRC 内嵌 `[language]`（按行序对应） | 本地 `AlbumImg` 缓存（480/120） |

翻译与原文的配对方式：网易云按时间戳精确配对；酷狗按歌词行序对应后转为同时间戳（±0.5 秒容差）；QQ 跨源译文按时间戳容差配对。
...
- **QQ 音乐**：
  - SMTC 不提供全局歌曲 ID，同名多版本依靠专辑名收敛，少数专辑缺失时可能匹配到其它版本；
  - 官方翻译歌词需登录态，程序改用跨源译文兜底：仅当同名歌曲原文重合率 ≥60% 且时间轴偏差中位数 ≤0.6 秒时才采用，因此部分冷门歌曲可能没有译文（此时只显示原文）。
- **酷狗音乐**：
  - 客户端不提供播放进度接口且界面进度不可稳定识别，进度按**本地时钟从切歌时刻累计**，无法感知暂停/继续，长时间播放会累积偏差；
  - 无法显示歌曲总时长（进度条为 `--:--`）；
  - 无法托盘运行（本程序对酷狗无此要求，但进度识别依赖歌名标题轮询）；
  - 译文来自歌词文件内嵌的 `[language]` 字段，部分歌曲只覆盖部分行（未覆盖的行不显示翻译）。
- 歌词在线获取（网易云 / QQ / 酷狗未命中本地缓存时）与 QQ 跨源译文查询需要网络连接。
